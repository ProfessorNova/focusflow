<script lang="ts">
    ...
    function RemoveTag(e:Event) {
        let _tag: string = e.currentTarget?.innerText;
...
<div class="flex flex-col h-full overflow-hidden gap-2 relative justify-evenly">
    <fieldset class="fieldset bg-base-200 border border-base-300 p-4 rounded-box overflow-y-scroll overflow-x-hidden flex-wrap flex flex-row min-h-25">
        <legend class="fieldset-legend">Selected tags</legend>
        {#each tags as tag}
            <div class="flex bg-neutral border border-success rounded px-3 py-1 w-fit h-fit gap-1 items-center justify-center">
    ...
    <fieldset class="fieldset bg-base-200 border border-base-300 p-4 rounded-box overflow-y-scroll overflow-x-hidden flex-wrap flex flex-row min-h-25">
        ...
        {#each availableTags as tag}
            <a href="#_" class="fieldset-label bg-neutral border border-info rounded px-3 py-1 w-fit" onclick="{AddTag}">{tag}</a>
        {/each}
    </fieldset>
</div>