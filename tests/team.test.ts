...
import * as teamModule from '$lib/server/objects/team'; // adjust the path if necessary
...
it('gets team members by team id', async () => {
  const mockTeam = [{ id: 5, name: 'Test', description: 'Test', users: [{ id: 123, username: 'user1' }], createdAt: new Date()}];
  __mockPrisma.team.findUnique.mockResolvedValue(mockTeam);
